...
export class NotionDashboard {
    ...
    try {
      await this.notion.pages.create({
        ...
        properties: {
          ...
          'From': {
            text: { content: this.truncateText(email.from || 'Unknown', 100) }
          },
          'Action': {
            select: { name: action }
          },
          ...
          'Reason': {
            text: { content: this.truncateText(reason || '', 100) }
          },
          'Date': {
  ...
  async logReply(originalSubject, command, actionTaken) {
    try {
      if (!this.databaseIds.replies) {
        console.warn('NOTION_REPLIES_DB not configured, skipping reply log');
        return;
      }
      await this.notion.pages.create({
        ...
        properties: {
          ...
          'Command': {
            text: { content: this.truncateText(command, 200) }
          },
          'Action Taken': {
            text: { content: this.truncateText(actionTaken, 200) }
          },
          'Date': {
            date: { start: new Date().toISOString() }
          },
      ...
      console.error('Reply data that failed:', { originalSubject, command, actionTaken });
    }
  }

  async updateStats(stats) {
    try {
      if (!this.databaseIds.stats) {
        console.warn('NOTION_STATS_DB not configured, skipping stats update');
        return;
      }
      ...
      if (existing.results.length > 0) {
        ...
        await this.notion.pages.create({
          ...
          properties: {
            'Title': {
              title: [{ text: { content: `Daily Stats - ${today}` } }]
            },
            'Date': {
              date: { start: today }
            },
      ...
      console.error('Stats that failed to update:', stats);
    }
  }
  ...
  async updatePreferences(preferences) {
    try {
      if (!this.databaseIds.preferences) {
        console.warn('NOTION_PREFERENCES_DB not configured, skipping preferences update');
        return;
      }
      ...
      console.error('Preferences that failed to update:', preferences);
    }
  }
    ...
    try {
      ...
      for (const page of response.results) {
        ...
        const rule = {
          ...
          emailPattern: properties['Email Pattern']?.text?.content || '',
          subjectPattern: properties['Subject Pattern']?.text?.content || '',
    ...
    try {
      await this.notion.pages.create({
        ...
        properties: {
          ...
          'Reason': {
            text: { content: this.truncateText(`Manual rule: ${rule.name}`, 100) }
          },
          'Date': {
            date: { start: new Date().toISOString() }
          }
  ...
  // Get processed email IDs from Notion activity log
  async getProcessedEmailIds(type = "emails") {
    try {
      const response = await this.notion.databases.query({
        database_id: this.databaseIds.activity,
        filter: {
          and: [
            {
              property: 'Action',
              select: {
                equals: type === "emails" ? 'forwarded' : 'reply_processed'
              }
            },
            {
              property: 'Date',
              date: {
                after: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString() // Last 7 days
              }
            }
          ]
        },
      ...
      // Extract email IDs from the activity log
      const processedIds = response.results
        .map(page => {
          const emailSubject = page.properties?.Subject?.title?.[0]?.text?.content;
          const emailFrom = page.properties?.From?.email?.email || page.properties?.From?.text?.content;
          
          // Create a simple ID from subject + from to match against
          if (emailSubject && emailFrom) {
            return `${emailFrom}-${emailSubject}`.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
          }
          return null;
        })
        .filter(Boolean);

      return [...new Set(processedIds)]; // Remove duplicates
    } catch (error) {
      console.error('Error getting processed email IDs from Notion:', error);
      return [];
    }
  }
}